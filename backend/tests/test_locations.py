...
class TestPostcodeDecoding:
    ...
    def test_postcode_with_no_known_area_mapping_falls_back_honestly(self):
        """TN (Tunbridge Wells) isn't mapped to any canonical city - should fall to Other UK, not guess wrong."""
        assert categorize_location("TN12AB") == "Other UK"

    def test_second_pass_postcode_areas_decode_correctly(self):
        """Areas added in the second expansion pass - see module docstring, points 4 and 5."""
        cases = [
            ("WF12DH", "Wakefield"),  # previously the documented gap - now mapped
            ("SL41AB", "Slough"), ("SG11AA", "Stevenage"), ("AL13AB", "St Albans"),
            ("RH101AB", "Crawley"), ("DH11AB", "Durham"), ("NP101AB", "Newport"),
        ]
        for raw, expected in cases:
            assert categorize_location(raw) == expected, f"{raw!r} should decode to {expected!r}"

    def test_outer_london_postcode_areas_fold_into_london(self):
        """Greater London operates as one job-market brand - see module docstring, point 4."""
        cases = ["CR01AB", "BR11AB", "DA11AB", "EN11AB", "HA11AB", "KT11AB", "TW11AB", "UB11AB", "IG11AB", "RM11AB"]
        for raw in cases:
            assert categorize_location(raw) == "London", f"{raw!r} should fold into London"
...
class TestNewTowns:
    ...
    def test_towns_added_in_second_expansion_pass(self):
        """See module docstring, point 5, for how these were chosen."""
        cases = [
            ...
            "Carlisle", "Darlington", "Taunton", "Salisbury", "Newport",
        ]
        for town in cases:
            assert categorize_location(town) == town, f"{town!r} should categorize as itself"
...
class TestNoRegression:
    ...
    def test_genuine_unknowns_still_fall_back_honestly(self):
        ...
        assert categorize_location("Somewhere Vague") == "Other UK"
        assert categorize_location("Tunbridge Wells, Kent") == "Other UK"  # documented known gap
