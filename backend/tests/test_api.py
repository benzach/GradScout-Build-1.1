...
class TestCriteria:
    ...
    def test_cannot_access_another_users_criteria(self, session, auth_headers):
        r = client.post("/criteria", json={"label": "Test search", "keywords": ["x"]}, headers=auth_headers)
    ...
    def test_update_is_partial(self, auth_headers):
        r = client.post("/criteria", json={"label": "Test search", "keywords": ["a"], "locations": ["London"]}, headers=auth_headers)
        criteria_id = r.json()["id"]
    ...
    def test_label_is_mandatory(self, auth_headers):
        r = client.post("/criteria", json={"keywords": ["x"]}, headers=auth_headers)
        assert r.status_code == 422

    def test_whitespace_only_label_rejected(self, auth_headers):
        """A label of just spaces would look identical to a blank one everywhere it's displayed."""
        r = client.post("/criteria", json={"label": "   ", "keywords": ["x"]}, headers=auth_headers)
        assert r.status_code == 422

    def test_label_is_trimmed(self, auth_headers):
        r = client.post("/criteria", json={"label": "  Grad roles  ", "keywords": ["x"]}, headers=auth_headers)
        assert r.status_code == 201
        assert r.json()["label"] == "Grad roles"

    def test_excluded_keywords_round_trip(self, auth_headers):
        r = client.post(
            "/criteria",
            json={"label": "Grad roles", "keywords": ["graduate"], "excluded_keywords": ["senior", "manager"]},
            headers=auth_headers,
        )
        assert r.status_code == 201
        assert r.json()["excluded_keywords"] == ["senior", "manager"]

    def test_excluded_keywords_defaults_to_empty(self, auth_headers):
        r = client.post("/criteria", json={"label": "Grad roles", "keywords": ["x"]}, headers=auth_headers)
        assert r.status_code == 201
        assert r.json()["excluded_keywords"] == []

    def test_excluded_keywords_can_be_updated(self, auth_headers):
        r = client.post("/criteria", json={"label": "Grad roles", "keywords": ["x"]}, headers=auth_headers)
        criteria_id = r.json()["id"]
        r = client.patch(f"/criteria/{criteria_id}", json={"excluded_keywords": ["intern"]}, headers=auth_headers)
        assert r.status_code == 200
        assert r.json()["excluded_keywords"] == ["intern"]

    def test_cannot_blank_out_label_via_update(self, auth_headers):
        r = client.post("/criteria", json={"label": "Grad roles", "keywords": ["x"]}, headers=auth_headers)
        criteria_id = r.json()["id"]
        r = client.patch(f"/criteria/{criteria_id}", json={"label": "  "}, headers=auth_headers)
        assert r.status_code == 422

    def test_delete(self, auth_headers):
        r = client.post("/criteria", json={"label": "Test search", "keywords": ["a"]}, headers=auth_headers)
        criteria_id = r.json()["id"]
...
class TestFeed:
    ...
    def test_feed_only_returns_matching_jobs(self, session, auth_headers):
        ...
        client.post("/criteria", json={"label": "Test search", "keywords": ["software"], "locations": ["London"]}, headers=auth_headers)
    ...
    def test_feed_materializes_matches_idempotently(self, session, auth_headers):
        ...
        client.post("/criteria", json={"label": "Test search", "keywords": ["analyst"]}, headers=auth_headers)
    ...
    def test_salary_min_excludes_lower_but_keeps_unparsed(self, session, auth_headers):
        ...
        client.post("/criteria", json={"label": "Test search", "salary_min": 30000}, headers=auth_headers)
        r = client.get("/feed", headers=auth_headers)
...
class TestFavourites:
    ...
    def test_favourites_only_filter(self, session, auth_headers):
        ...
        client.post("/criteria", json={"label": "Test search", "keywords": ["graduate"], "locations": ["London"]}, headers=auth_headers)
