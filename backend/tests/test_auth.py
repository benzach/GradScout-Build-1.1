...
class TestAccountSettings:
    def test_email_digest_defaults_to_enabled(self, session):
        r = client.post("/auth/signup", json={"email": "settings1@example.com", "password": "correct-password"})
        assert r.json()["user"]["email_digest_enabled"] is True

    def test_can_disable_email_digest(self, session):
        r = client.post("/auth/signup", json={"email": "settings2@example.com", "password": "correct-password"})
        headers = {"Authorization": f"Bearer {r.json()['access_token']}"}

        r = client.patch("/auth/me", json={"email_digest_enabled": False}, headers=headers)
        assert r.status_code == 200
        assert r.json()["email_digest_enabled"] is False

        user = session.query(User).filter_by(email="settings2@example.com").first()
        assert user.email_digest_enabled is False

    def test_can_re_enable_email_digest(self, session):
        r = client.post("/auth/signup", json={"email": "settings3@example.com", "password": "correct-password"})
        headers = {"Authorization": f"Bearer {r.json()['access_token']}"}
        client.patch("/auth/me", json={"email_digest_enabled": False}, headers=headers)

        r = client.patch("/auth/me", json={"email_digest_enabled": True}, headers=headers)
        assert r.json()["email_digest_enabled"] is True

    def test_unauthenticated_request_rejected(self, session):
        r = client.patch("/auth/me", json={"email_digest_enabled": False})
...
class TestAccountDeletion:
    ...
    def test_deletion_cascades_to_everything(self, session):
        ...
        client.post("/criteria", json={"label": "Test search", "keywords": ["graduate"]}, headers=headers)
