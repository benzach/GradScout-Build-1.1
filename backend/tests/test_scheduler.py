...
from app.scheduler import run_scheduled_cycle, run_weekly_digest_job
...
def test_weekly_digest_job_runs_without_raising_when_unconfigured(session):
    """Same principle as the pipeline/matching tests above: the scheduler's wrapper around a real function shouldn't crash the scheduler thread, whatever that function returns."""
    summary = run_weekly_digest_job()
    assert summary["configured"] is False


def test_weekly_digest_job_captures_exceptions(session):
    with patch("app.scheduler.send_weekly_digests", side_effect=RuntimeError("simulated digest failure")):
        summary = run_weekly_digest_job()  # must not raise
    assert summary["fatal_error"] == "simulated digest failure"
