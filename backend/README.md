...
3. Run the migrations against that same database, in order, from your
   ...
   Railway's network). Same loop as Local Setup above, just pointed at
   Railway's connection string instead of localhost — deliberately not
   a hardcoded list of individual `psql` commands here any more, since
   that list silently went stale in the past (fell three migrations
   behind and would have skipped 0008–0010 for anyone following it
   literally). The loop can't go stale the same way — it always picks
   up every file in migrations/, including ones added after this README
   was last edited:
   ```bash
   for f in migrations/*.sql; do psql "<railway-postgres-public-connection-string>" -f "$f"; done
   ```
4. Add the remaining Railway environment variables: `JWT_SECRET_KEY`
   ...
   `REED_API_KEY`, `JOOBLE_API_KEY`). See `.env.example` for the full,
   up-to-date list, including optional ones (`ANTHROPIC_API_KEY`,
   `SCRAPER_CONTACT_URL`/`SCRAPER_CONTACT_EMAIL`) — that file is the
   source of truth for what exists; this README only calls out the
   ones that are required just to get the app running at all.
