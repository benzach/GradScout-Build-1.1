...
the right tradeoff. (A third-party email provider — Resend — WAS added
alongside this file, for the weekly digest in app/email_digest.py; that
doesn't change the calculus here specifically, since verification and
password-reset are their own separate flows with their own security
considerations, not something that falls out for free once *an* email
integration exists. Still flagged explicitly as a gap to revisit before
a wider launch, not something to find by accident later.)
"""
...
from app.schemas import AccountDeleteRequest, LoginRequest, SignupRequest, TokenResponse, UserOut, UserSettingsUpdate
...
def update_settings(
    ...
    """Account-level preferences — currently just email_digest_enabled (see Settings.jsx's notifications section)."""
