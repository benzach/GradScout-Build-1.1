...
class User(Base):
    ...
    created_at = Column(DateTime(timezone=True), default=lambda: datetime.now(timezone.utc))
    # See migrations/0013_email_digest.sql for the full reasoning.
...
class SearchCriteria(Base):
    ...
    excluded_keywords = Column(ARRAY(Text), nullable=False, default=list)
...
class Job(Base):
    ...
    created_at = Column(DateTime(timezone=True), default=lambda: datetime.now(timezone.utc))
    # NULL = still active. Set by app.storage.sweep_expired_jobs() once
    # none of this job's sources have been re-confirmed present in a
    # while — see migrations/0011_job_expiry.sql for the full reasoning.
    # Un-set automatically (see app.storage.process_scraped_job) if a
    # previously-expired job reappears in a fresh scrape.
