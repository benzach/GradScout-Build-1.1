...
def run_pipeline(session: Session) -> dict:
    ...
    # Run once per full cycle (across all sources), not per-source - a
    # job's staleness depends on ALL of its sources, not just whichever
    # one happened to trigger this check, and there's no benefit to
    # running it more than once here.
    summary["jobs_expired"] = sweep_expired_jobs(session)
...
if __name__ == "__main__":
    ...
    print(f"Jobs newly marked expired: {result['jobs_expired']}")
    if result["failures"]:
        print("Failures:")
