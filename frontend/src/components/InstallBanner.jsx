...
export default function InstallBanner() {
  ...
  return (
    <div className="fixed bottom-0 inset-x-0 z-50 bg-primary-900 text-background px-4 py-3 flex items-center gap-3 shadow-lg">
      {platform === 'ios' ? (
      ...
      <div className="flex items-center gap-2 shrink-0">
        {platform === 'android' && (
          <button
            ...
            className="text-sm font-semibold bg-accent-300 text-primary-900 px-3 py-1.5 rounded-lg hover:bg-accent-400 transition-colors"
          >
            Install
          </button>
        )}
        <button onClick={dismiss} aria-label="Dismiss" className="text-background/70 hover:text-background p-1">
          <X size={18} />
        </button>
      </div>
    </div>
